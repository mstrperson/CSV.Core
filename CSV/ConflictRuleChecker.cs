...
public class UserResolveConflictRule : IConflictRule
{
    ...
    {
        Console.WriteLine($"Conflicting Data in column '{conflictColumn}'.  Choose which data to keep.");
        Console.WriteLine($"1) {dataA[conflictColumn]}\n2) {dataB[conflictColumn]}\n3) *something else*");
        Console.Write(">> ");
...
public class YesBeatsNoConflictRule : IConflictRule
{
    ...
    public string Resolve(Row dataA, Row dataB, string conflictColumn)
    {
        ...
        if (NextRule is null) throw new ConflictRuleException("YesBeatsNo rule does not apply and NextRule is not defined.");
