...
namespace Csv
{
    ...
    public class ExtendedCSV : CSV
    {
        ...
        public ExtendedCSV(Stream inputStream, List<string>? uidfields = default) : base(inputStream)
        {
        ...
        {
            this.UniqueFields = uidfields == null ? new List<string>() : uidfields;
        }
            ...
            foreach(string key in keys)
            {
                if(!UniqueFields.Contains(key))
                {
                    ...
                    if (!destination[key].Equals(row[key]) && !string.IsNullOrWhiteSpace(row[key]))
                        destination[key] = ConflictRule?.Resolve(destination, row, key) ?? "Unresolved Conflict";
                    ...
                    if(CompareUniqueFields(rowA, rowB))
                    {
                        foreach(string column in rowB.Keys)
                        {
                            ...
                            else if(!string.IsNullOrWhiteSpace(rowB[column]) && !rowA[column].Equals(rowB[column]))
                            {
                                ...
                                rowA[column] = ConflictRule?.Resolve(rowA, rowB, column) ?? "Unresolved Conflict";
                            }
                        }
                    }
                }
